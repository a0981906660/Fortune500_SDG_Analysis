...
#'                          log(at)                  (firm size, log total assets)
...
#'     - Added `log(at)` back as an additional firm size control variable
...
#    Key columns: gvkey, fyear, conm, at, revt, emp,
...
firm_global <- firm_global %>%
  select(gvkey, fyear, conm, curcd, at, revt, emp,
...
firm_all <- firm_us %>%
  ...
  mutate(
    ...
    # log(at): at is total assets in millions USD after currency conversion
    # add a small constant (0.01) before log to avoid log(0) = -Inf
    log_at  = log(at  + 0.01)
  )
...
cat("Firms with valid log_at   : ",
    df_merged %>% filter(!is.na(log_at))  %>% distinct(gvkey) %>% nrow(), "\n")
...
#           + log_at                     # control: firm size (log total assets)
...
# -----------------------------------------------------------------------------
reg_roa_A <- feols(
  roa ~ colocate_index_complementary + colocate_index_independent + colocate_index_pressure
  + log_emp + log_at | csw0(fyear, naics2),
  vcov = "HC1",
  data = df_merged
)
...
reg_roa_B <- feols(
  roa ~ colocate_index_complementary + colocate_index_independent + colocate_index_pressure
  + log_emp + log_at | csw0(fyear, sic2),
  vcov = "HC1",
  data = df_merged
)
...
save_reg_table(reg_roa_A, "%tab_reg_roa_fullsample_naics2_logemp_logat")
save_reg_table(reg_roa_B, "%tab_reg_roa_fullsample_sic2_logemp_logat")
save_reg_table(reg_roa_C, "%tab_reg_roa_mining_naics2_logemp_logat")
save_reg_table(reg_roa_D, "%tab_reg_roa_mining_sic2_logemp_logat")

# -----------------------------------------------------------------------------
# 6b. ROS as dependent variable
# -----------------------------------------------------------------------------
reg_ros_A <- feols(
  ros ~ colocate_index_complementary + colocate_index_independent + colocate_index_pressure
  + log_emp + log_at | csw0(fyear, naics2),
  vcov = "HC1",
  data = df_merged
)
...
save_reg_table(reg_ros_A, "%tab_reg_ros_fullsample_naics2_logemp_logat")
save_reg_table(reg_ros_B, "%tab_reg_ros_fullsample_sic2_logemp_logat")
save_reg_table(reg_ros_C, "%tab_reg_ros_mining_naics2_logemp_logat")
save_reg_table(reg_ros_D, "%tab_reg_ros_mining_sic2_logemp_logat")
...
# -----------------------------------------------------------------------------
reg_tobinq_A <- feols(
  tobin_q ~ colocate_index_complementary + colocate_index_independent + colocate_index_pressure
  + log_emp + log_at | csw0(fyear, naics2),
  vcov = "HC1",
...
save_reg_table(reg_tobinq_A, "%tab_reg_tobinq_fullsample_naics2_logemp_logat")
save_reg_table(reg_tobinq_B, "%tab_reg_tobinq_fullsample_sic2_logemp_logat")
save_reg_table(reg_tobinq_C, "%tab_reg_tobinq_mining_naics2_logemp_logat")
save_reg_table(reg_tobinq_D, "%tab_reg_tobinq_mining_sic2_logemp_logat")
...
save_reg_table(list(reg_roa_A,    reg_roa_C),    "%tab_reg_roa_combined_logemp_logat")
save_reg_table(list(reg_ros_A,    reg_ros_C),    "%tab_reg_ros_combined_logemp_logat")
save_reg_table(list(reg_tobinq_A, reg_tobinq_C), "%tab_reg_tobinq_combined_logemp_logat")
